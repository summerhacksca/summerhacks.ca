...
export default function AboutSection() {
	...
	const measureLayout = () => {
		const heights = paragraphRefs.current.map(
			...
			<div className="sticky top-0 content-stretch flex flex-col items-start p-[12px] shrink-0 w-full h-screen z-[3]">
				<div className="bg-[#fffbf6] content-stretch flex flex-col gap-16 items-center justify-center overflow-clip pb-16 md:pb-32 pt-24 md:pt-41 px-6 md:px-32 relative shrink-0 w-full h-[calc(100dvh-24px)]">
					...
					<div className="content-stretch flex flex-col gap-[48px] items-start relative shrink-0 w-full">
						<div
							ref={textWrapperRef}
							className="flex flex-col font-['Maison Neue:Medium',sans-serif] justify-end leading-[1.2] min-w-full not-italic relative shrink-0 text-[#ffcf98] text-[20px] md:text-[32px] text-left md:!text-justify tracking-[-0.64px] w-[min-content]"
...
function TextParagraphs({
	...
	return (
		<>
			{/* paragraph 0 */}
			<p ref={bindRef(0)} className="mb-0 font-medium">
			...
			<p
				ref={bindRef(1)}
				className="font-['Maison Neue:Book',sans-serif] mb-0"
			>
			...
			<p
				ref={bindRef(2)}
				className="font-['Maison Neue:Book',sans-serif]"
			>
			...
			<div className="flex gap-2">
				<p
					ref={bindRef(3)}
					className="leading-[1.2] font-['Maison Neue:Book',sans-serif]"
				>
					Let&apos;s build in golden hours.
					{/* <span className={`text-2xl ${dotColorClass}`}>●</span> */}
				</p>
				<Image
					alt=""
...
function BackgroundImages({ visible }: { visible: boolean }) {
	return (
		<div
			className={`pointer-events-none transition-opacity duration-700 ${visible ? "opacity-100" : "opacity-0"}`}
		>
			<div
				className="absolute flex items-center justify-center left-[-297px] size-[717px] top-[-388px]"
				style={
					{
			...
			<div
				className="absolute flex items-center justify-center right-[-311px] size-[717px] top-[644px]"
				style={
					{
						"--transform-inner-width": "0",
						"--transform-inner-height": "0",
			...
			<div
				className="absolute bottom-[-503px] flex items-center justify-center left-[-380px] size-[717px]"
				style={
					{
						"--transform-inner-width": "0",
						"--transform-inner-height": "0",
...
function ContinueButton() {
	return (
		<div className="content-stretch flex flex-col items-center justify-end relative shrink-0">
			<a
				href="#info"
				className="backdrop-blur-[1px] backdrop-filter bg-[#ffefdd] content-stretch flex gap-[10px] h-[52px] items-center overflow-clip px-[16px] py-0 relative rounded-[100px] shrink-0"
			>
